...
func NewStore(client *awsclient.Client, env string) (*Store, error) {
	if env == "" {
...
func (s *Store) Put(ctx context.Context, material usecase.KeyMaterial) error {
	if s == nil || s.client == nil {
		return errors.New("aws store not configured")
	}
	...
	return s.client.CreateSecret(ctx, secretID, encoded)
}
