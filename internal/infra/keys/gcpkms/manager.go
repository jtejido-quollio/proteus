...
func NewManager(client *gcpclient.Client, env string) (*Manager, error) {
	if env == "" {
...
func (m *Manager) Sign(ctx context.Context, ref domain.KeyRef, payload []byte) ([]byte, error) {
	if m == nil || m.client == nil {
		return nil, errors.New("gcp manager not configured")
	}
	...
	secretBytes, err := m.client.AccessSecret(ctx, secretID)
	if err != nil {
		return nil, err
	}
